...
- 第 6 章 PNG 由 `scripts/plot_thesis_ch6_figures.py` 生成。脚本会统一调用 `scripts/figure_fonts.py` 注册中文字体；若换机后仍出现中文方框，可安装微软雅黑/黑体/Noto Sans CJK，或用环境变量 `THESIS_CJK_FONT` 指向可用的 `.ttf/.ttc/.otf` 字体文件后重跑出图命令。
